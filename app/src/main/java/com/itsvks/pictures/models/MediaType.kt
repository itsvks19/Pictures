...
enum class MediaType(
  ...
  companion object {
    ...
    fun fromMediaStoreValue(value: Int) = entries.first { value == it.mediaStoreValue }

    fun fromMimeType(mimeType: String) = when {
      mimeType.startsWith("image/") -> IMAGE
      mimeType.startsWith("video/") -> VIDEO
      mimeType in dashMimeTypes || mimeType in hlsMimeTypes || mimeType in smoothStreamingMimeTypes -> VIDEO
      else -> null
    }
  }
}