...
@Composable
fun rememberEncryptedScrollbarSettings(
...
@Composable
fun EncryptedTimelineScroller(
  state: LazyGridState,
  modifier: Modifier = Modifier,
  mappedData: SnapshotStateList<EncryptedMediaItem>,
  headers: SnapshotStateList<EncryptedMediaItem.Header>,
  settings: ScrollbarSettings = rememberEncryptedScrollbarSettings(headers),
        ...
        val currentDate by remember(mappedData, index) {
          derivedStateOf {
            mappedData.getOrNull((index + 1).coerceAtMost(mappedData.size - 1))?.let { item ->
              when (item) {
                is EncryptedMediaItem.MediaViewItem -> item.media.timestamp.getDate(
                  stringToday = stringToday,
                  stringYesterday = stringYesterday
                )

                is EncryptedMediaItem.Header -> item.text
              }
            }
          }
        }
